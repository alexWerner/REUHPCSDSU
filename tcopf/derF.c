...
PetscErrorCode setupConstraints(DM net, PetscInt nb, PetscInt ng, Vec *x, Vec *xmin, Vec *xmax)
{
  ...
  ierr = MakeVector(x, xSize * timeSteps);CHKERRQ(ierr);
  ierr = MakeVector(xmin, xSize * timeSteps);CHKERRQ(ierr);
  ierr = MakeVector(xmax, xSize * timeSteps);CHKERRQ(ierr);
  ...
  for (PetscInt i = vStart; i < vEnd; i++)
  {
    ...
    for (kk = 0; kk < numComponents; kk++)
    {
      ...
      if (key == 1)
      {
        ...
        {
          ierr = VecSetValue(*x, bus->internal_i + (j * xSize), 0, INSERT_VALUES);CHKERRQ(ierr);
          ierr = VecSetValue(*x, bus->internal_i + nb + (j * xSize), 1, INSERT_VALUES);CHKERRQ(ierr);
        }
        ...
        if (bus->ide == 3)
        {
          for (int j = 0; j < timeSteps; j++)
          {
            ierr = VecSetValue(*xmin, bus->internal_i + (j * xSize), bus->va, INSERT_VALUES);CHKERRQ(ierr);
            ierr = VecSetValue(*xmax, bus->internal_i + (j * xSize), bus->va, INSERT_VALUES);CHKERRQ(ierr);
          }
        }
        else
        {
          for (int j = 0; j < timeSteps; j++)
          {
            ierr = VecSetValue(*xmin, bus->internal_i + (j * xSize), PETSC_INFINITY * -1, INSERT_VALUES);CHKERRQ(ierr);
            ierr = VecSetValue(*xmax, bus->internal_i + (j * xSize), PETSC_INFINITY, INSERT_VALUES);CHKERRQ(ierr);
          }
        }

        for (int j = 0; j < timeSteps; j++)
        {
          ierr = VecSetValue(*xmin, bus->internal_i + nb + (j * xSize), 0.9, INSERT_VALUES);CHKERRQ(ierr);
          ierr = VecSetValue(*xmax, bus->internal_i + nb + (j * xSize), 1.1, INSERT_VALUES);CHKERRQ(ierr);
        }
      }
      else if (key == 2)
      {
        ...
        for (int j = 0; j < timeSteps; j++)
        {
          ierr = VecSetValue(*x, gen->idx + nb * 2 + (j * xSize), (gen->pt + gen->pb) / 200, INSERT_VALUES);CHKERRQ(ierr);
          ierr = VecSetValue(*x, gen->idx + nb * 2 + ng + (j * xSize), (gen->qt + gen->qb) / 200, INSERT_VALUES);CHKERRQ(ierr);

          ierr = VecSetValue(*xmin, gen->idx + nb * 2 + (j * xSize), gen->pb / 100, INSERT_VALUES);CHKERRQ(ierr);
          ierr = VecSetValue(*xmin, gen->idx + nb * 2 + ng + (j * xSize), gen->qb / 100, INSERT_VALUES);CHKERRQ(ierr);

          ierr = VecSetValue(*xmax, gen->idx + nb * 2 + (j * xSize), gen->pt / 100, INSERT_VALUES);CHKERRQ(ierr);
          ierr = VecSetValue(*xmax, gen->idx + nb * 2 + ng + (j * xSize), gen->qt / 100, INSERT_VALUES);CHKERRQ(ierr);
        }
      }
    }
  }
                                   ...
                                   Mat *dSf_dVa, Mat *dSf_dVm, Mat *dSt_dVm, Mat *dSt_dVa, Vec *Sf, Vec *St)
{
  ...
  ierr = MakeVector(gn, nb * 2 * timeSteps);CHKERRQ(ierr);
  ...
  for (int ts = 0; ts < timeSteps; ts++)
  {
    ...
    Vec xIt, xminIt, xmaxIt;
    ierr = getVecIndices(x, ts * xSize, (ts + 1) * xSize, &xIt);CHKERRQ(ierr);
    ierr = getVecIndices(xmin, ts * xSize, (ts + 1) * xSize, &xminIt);CHKERRQ(ierr);
    ierr = getVecIndices(xmax, ts * xSize, (ts + 1) * xSize, &xmaxIt);CHKERRQ(ierr);

    // VecView(x, PETSC_VIEWER_STDOUT_WORLD);
    Vec Pg, Qg, Vm, Va;
    ierr = getVecIndices(xIt, 0, nb, &Va);CHKERRQ(ierr);
    ierr = getVecIndices(xIt, nb, nb * 2, &Vm);CHKERRQ(ierr);
    ierr = getVecIndices(xIt, nb * 2, xSize - ng, &Pg);CHKERRQ(ierr);
    ierr = getVecIndices(xIt, xSize - ng, xSize, &Qg);CHKERRQ(ierr);
    ...
    Vec gnIt;
    ierr = MakeVector(&gnIt, nb * 2);CHKERRQ(ierr);
    ...
    {
      VecSetValue(gnIt, i, PetscRealPart(misArr[i - min]), INSERT_VALUES);CHKERRQ(ierr);
      VecSetValue(gnIt, i + nb, PetscImaginaryPart(misArr[i - min]), INSERT_VALUES);CHKERRQ(ierr);

      VecSetValue(*gn, i + nb * 2 * ts, PetscRealPart(misArr[i - min]), INSERT_VALUES);CHKERRQ(ierr);
      VecSetValue(*gn, i + nb + nb * 2 * ts, PetscImaginaryPart(misArr[i - min]), INSERT_VALUES);CHKERRQ(ierr);
    }
    ...
    ierr = VecDuplicate(VfInd, &Sf[ts]);CHKERRQ(ierr);
    ierr = VecPointwiseMult(Sf[ts], VfInd, YfV);CHKERRQ(ierr);
    ...
    ierr = VecDuplicate(VtInd, &St[ts]);CHKERRQ(ierr);
    ierr = VecPointwiseMult(St[ts], VtInd, YtV);CHKERRQ(ierr);
    ierr = VecRestoreSubVector(V, isTV, &VtInd);CHKERRQ(ierr);

    //hn = [Sf .* conj(Sf) - flow_max;
    //      St .* conj(St) - flow_max ];
    Vec hnIt;
    ierr = MakeVector(&hnIt, nl2 * 2);CHKERRQ(ierr);
    Vec SfConj, StConj;
    ierr = VecDuplicate(Sf[ts], &SfConj);CHKERRQ(ierr);
    ierr = VecDuplicate(St[ts], &StConj);CHKERRQ(ierr);
    ierr = VecCopy(Sf[ts], SfConj);CHKERRQ(ierr);
    ierr = VecCopy(St[ts], StConj);CHKERRQ(ierr);
    ierr = VecConjugate(SfConj);CHKERRQ(ierr);
    ierr = VecConjugate(StConj);CHKERRQ(ierr);

    ierr = VecPointwiseMult(SfConj, Sf[ts], SfConj);CHKERRQ(ierr);
    ierr = VecPointwiseMult(StConj, St[ts], StConj);CHKERRQ(ierr);
    ...
    for (PetscInt i = min; i < max; i++)
    {
      VecSetValue(hnIt, i, SfArr[i - min], INSERT_VALUES);CHKERRQ(ierr);
      VecSetValue(hnIt, i + nl2, StArr[i - min], INSERT_VALUES);CHKERRQ(ierr);

      VecSetValue(*hn, i + ts * nl2 * 2, SfArr[i - min], INSERT_VALUES);CHKERRQ(ierr);
      VecSetValue(*hn, i + nl2 + ts * nl2 * 2, StArr[i - min], INSERT_VALUES);CHKERRQ(ierr);
    }
    ...
    ierr = VecAssemblyBegin(hnIt);CHKERRQ(ierr);
    ierr = VecAssemblyEnd(hnIt);CHKERRQ(ierr);
    ...
    ierr = dSMat(&dSf_dVa[ts], PETSC_i, -1, nl2, nb, diagIf, diagVf, V, YfIl, isFV, diagV);
    ierr = dSMat(&dSf_dVm[ts], 1, 1, nl2, nb, diagIf, diagVf, Vnorm, YfIl, isFV, diagVnorm);
    ierr = dSMat(&dSt_dVa[ts], PETSC_i, -1, nl2, nb, diagIt, diagVt, V, YtIl, isTV, diagV);
    ierr = dSMat(&dSt_dVm[ts], 1, 1, nl2, nb, diagIt, diagVt, Vnorm, YtIl, isTV, diagVnorm);
    ...
    //Sf = V(f1) .* conj(If);
    //St = V(t1) .* conj(It);
    ierr = VecDestroy(&Sf[ts]);CHKERRQ(ierr);
    ierr = VecDestroy(&St[ts]);CHKERRQ(ierr);
    ...
    ierr = VecDuplicate(VisF, &Sf[ts]);CHKERRQ(ierr);
    ierr = VecDuplicate(VisT, &St[ts]);CHKERRQ(ierr);

    ierr = VecPointwiseMult(Sf[ts], VisF, If);CHKERRQ(ierr);
    ierr = VecPointwiseMult(St[ts], VisT, It);CHKERRQ(ierr);
    ...
    ierr = makeDiagonalMatRI(&dAf_dPf, Sf[ts], nl2, 'r', 2);CHKERRQ(ierr);
    ierr = makeDiagonalMatRI(&dAf_dQf, Sf[ts], nl2, 'i', 2);CHKERRQ(ierr);
    ierr = makeDiagonalMatRI(&dAt_dPt, St[ts], nl2, 'r', 2);CHKERRQ(ierr);
    ierr = makeDiagonalMatRI(&dAt_dQt, St[ts], nl2, 'i', 2);CHKERRQ(ierr);
    ...
    ierr = matRealPMatImag(&dAf_dVm, dAf_dPf, dAf_dQf, dSf_dVm[ts]);CHKERRQ(ierr);
    ierr = matRealPMatImag(&dAf_dVa, dAf_dPf, dAf_dQf, dSf_dVa[ts]);CHKERRQ(ierr);
    ierr = matRealPMatImag(&dAt_dVm, dAt_dPt, dAt_dQt, dSt_dVm[ts]);CHKERRQ(ierr);
    ierr = matRealPMatImag(&dAt_dVa, dAt_dPt, dAt_dQt, dSt_dVa[ts]);CHKERRQ(ierr);
    ...
    ierr = VecWAXPY(absDiff, -1, xminIt, xmaxIt);CHKERRQ(ierr);
    ...
    IS ibx;
    Vec ibxVec[3] = {absDiff, xmaxIt, xminIt};
    ...
    //be = xmax(ieq, 1);
    Vec be;
    ierr = getSubVector(xmaxIt, ieq, &be);CHKERRQ(ierr);
    ...
    ierr = getSubVector(xmaxIt, ilt, &maxIlt);CHKERRQ(ierr);
    ierr = getSubVector(xmaxIt, ibx, &maxIbx);CHKERRQ(ierr);
    ierr = getSubVector(xminIt, igt, &minIgt);CHKERRQ(ierr);
    ierr = getSubVector(xminIt, ibx, &minIbx);CHKERRQ(ierr);
    ...
    ierr = MatMult(Ai, xIt, Aix);CHKERRQ(ierr);
    ierr = VecAXPY(Aix, -1, bi);CHKERRQ(ierr);

    Vec hVecs[2] = {hnIt, Aix};
    PetscInt hnN;
    ...
    PetscInt * hInd = intArray2(min, max);
    PetscInt *hGInd = intArray2(min + ts * (hnN + iltN + igtN + 2 * ibxN), max + ts * (hnN + iltN + igtN + 2 * ibxN));
    PetscScalar *hVals;
    ...
    PetscInt gnN;
    ierr = VecGetSize(gnIt, &gnN);CHKERRQ(ierr);
    ierr = stackNVectors(&gIt, gVecs, 2, gnN + ieqN);CHKERRQ(ierr);

    if(ts == 0)
      ierr = MakeVector(g, (gnN + ieqN) * timeSteps);CHKERRQ(ierr);

    ierr = VecGetOwnershipRange(gIt, &min, &max);CHKERRQ(ierr);
    PetscInt * gInd = intArray2(min, max);
    PetscInt *gGInd = intArray2(min + ts * (gnN + ieqN), max + ts * (gnN + ieqN));
    PetscScalar *gVals;
    ierr = PetscMalloc1(max - min, &gVals);CHKERRQ(ierr);
    ierr = VecGetValues(gIt, max - min, gInd, gVals);CHKERRQ(ierr);
    ierr = VecSetValues(*g, max - min, gGInd, gVals, INSERT_VALUES);CHKERRQ(ierr);
    ierr = PetscFree(gVals);
    ierr = PetscFree(gInd);CHKERRQ(ierr);
    ierr = PetscFree(gGInd);CHKERRQ(ierr);
    ...
    PetscInt dhnR, dhnC, AiR, AiC;
    ierr = MatGetSize(dhn, &dhnR, &dhnC);CHKERRQ(ierr);
    ierr = MatGetSize(AiT, &AiR, &AiC);CHKERRQ(ierr);
    if(ts == 0)
      ierr = makeSparse(&dhPre, xSize * timeSteps, (dhnC + AiC) * timeSteps, dhnC + AiC, dhnC + AiC);CHKERRQ(ierr);

    ierr = matJoinMatWidth(&dhIt, dhn, AiT);CHKERRQ(ierr);

    ierr = tileDiag(dhPre, dhIt, ts);CHKERRQ(ierr);
    ...
    PetscInt dgnR, dgnC, AeR, AeC;
    ierr = MatGetSize(dgn, &dgnR, &dgnC);CHKERRQ(ierr);
    ierr = MatGetSize(AeT, &AeR, &AeC);CHKERRQ(ierr);
    if(ts == 0)
      ierr = makeSparse(dg, xSize * timeSteps, (dgnC + AeC) * timeSteps, dgnC + AeC, dgnC + AeC);CHKERRQ(ierr);

    ierr = matJoinMatWidth(&dgIt, dgn, AeT);CHKERRQ(ierr);

    ierr = tileDiag(*dg, dgIt, ts);CHKERRQ(ierr);
    PetscLogStagePop();
    ...
    ierr = VecDestroy(&xIt);CHKERRQ(ierr);
    ierr = VecDestroy(&xminIt);CHKERRQ(ierr);
    ierr = VecDestroy(&xmaxIt);CHKERRQ(ierr);
    ierr = VecDestroy(&hIt);CHKERRQ(ierr);
    ierr = VecDestroy(&gIt);CHKERRQ(ierr);
  }
  ierr = MatAssemblyBegin(*dg, MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
  ierr = MatAssemblyBegin(dhPre, MAT_FINAL_ASSEMBLY);CHKERRQ(ierr);
                     ...
                     Mat diagIf, Mat diagVf, Vec V, Mat YfIl, IS isFV, Mat diagV)
{
  ...
  ierr = ISGetIndices(isFV, &f1Arr);CHKERRQ(ierr);
  ierr = VecGetOwnershipRange(VisF, &min, &max);CHKERRQ(ierr);
  for (PetscInt i = min; i < max; i++)
  {
